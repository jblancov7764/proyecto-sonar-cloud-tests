...
class Vista_actividad(QWidget):
    ...
    def mostrar_gastos_por_actividad(self, actividad, gastos):
        ...
        self.gastos = gastos
        self.id_actividad = actividad.id
        ...
        self.viajeros = self.cuentasClaras.darViajerosAsociadosEnActividad(self.id_actividad)
        if len(self.viajeros) < 1:
            self.btn_aniadir_gasto.setEnabled(False)

        numero_fila = 1

        # Ciclo para llenar los gastos
        for gasto in self.gastos:
            viajero = self.cuentasClaras.darViajeroPorId(gasto.viajero)
            etiqueta_nombre = QLabel(viajero.nombre + " " + viajero.apellido)
            etiqueta_nombre.setWordWrap(True)
            self.distribuidor_actividades.addWidget(etiqueta_nombre, numero_fila, 0)

            etiqueta_fecha = QLabel(gasto.fecha.strftime('%m/%d/%Y'))
            ...
            btn_editar.clicked.connect(partial(self.editar_gasto, gasto))
        ...
        elemento_de_espacio = QSpacerItem(140, 360 - numero_fila * 40 if numero_fila * 40 <= 360 else 0,
    ...
    def mostrar_reporte_gastos_por_viajero(self):
        ...
        """
        self.hide()
        self.interfaz.mostrar_reporte_gastos_viajero(self.id_actividad)

    def crear_gasto(self):
        """
        Esta función ejecuta el diálogo para crear un gasto
        """
        dialogo = Dialogo_crear_gasto(self.interfaz.dar_viajeros(self.id_actividad), None, self.id_actividad)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.insertar_gasto(dialogo.texto_concepto.text(),
                                         str(dialogo.campo_fecha.date().currentDate().toPyDate().strftime("%d/%m/%Y")),
    ...
    def editar_gasto(self, gasto):
        ...
        """
        dialogo = Dialogo_crear_gasto(self.interfaz.dar_viajeros(self.id_actividad), gasto, self.id_actividad)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.editar_gasto(gasto, dialogo.texto_concepto.text(),
