...
class Dialogo_crear_actividad(QDialog):
    ...
    def __init__(self, actividad):
        ...
        self.editar = actividad is not None
    ...
    def guardar(self):
        ...
        self.resultado = 1
        if self.editar:
            self.cuentasClaras.editarActividad(self.actividad.id, self.texto_nombre.text(),
                                               self.actividad.estaTerminada)
        else:
            self.cuentasClaras.crearActividad(self.texto_nombre.text(), False)
        self.close()
